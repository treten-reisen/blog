    ...
    >
      <header className="flex flex-col space-y-2">
        <img
          ...
          className="h-52 lg:h-64 object-cover"
        />
        <h2 className="font-sans text-lg font-bold tracking-tight text-gray-700">
          {item.attributes.title}
        </h2>
      </header>
      <section className="font-sans text-gray-600 tracking-tight">
        {item.attributes.summary}
      </section>
      <footer className="self-end font-sans text-base tracking-tight text-gray-500">
        {dateFns.intlFormat(item.attributes.createdAt)}
      </footer>
    </a>
  )
}
